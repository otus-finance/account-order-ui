    ...
    markets.map(async (market) => {
      ...
      const liveBoards: LyraBoard[] =
        lyra.chainId === arbitrum.id
          ? parseMarketBoardsArb(name, market.liveBoardsMap)
...
const parseMarketBoardsArb = (
  marketName: string,
  boards: Record<number, any>
): LyraBoard[] => {
  return Object.values(boards).map((board) => {
    const { id, expiry, baseIv } = board;

    const expiryTimestamp = fromBigNumber(expiry);

    const strikes: Strike[] = board.strikes
      .filter((strike: LyraStrike) => strike.isDeltaInRange)
      .sort(sortStrikes);
    const name = formatBoardName(expiryTimestamp);
    return { name, id, expiryTimestamp, baseIv, strikes, marketName };
  });
