  ...
  return <>
    ...
    <div className='pb-2'>
      ...
      <div className='bg-black border border-zinc-800 py-4 p-2'>
        ...
        <div className="flex items-center justify-between px-2 pt-3">
          ...
          {
            AccountActionType.DEPOSIT === accountActionType && accountAllowance && accountAllowance > 0 ?
              <DebounceInput
                minLength={1}
                debounceTimeout={300}
                onChange={async (e) => {
          ...
          {
            AccountActionType.DEPOSIT === accountActionType && accountAllowance === 0 ?
              <DebounceInput
                minLength={1}
                debounceTimeout={300}
                onChange={async (e) => {
...

    {
      accountAllowance === 0 && AccountActionType.DEPOSIT === accountActionType ?
        <div onClick={() => approveQuote?.()} className="cursor-pointer border-2 border-emerald-600 hover:border-emerald-600 p-2 py-3 col-span-3 font-semibold text-sm text-white text-center rounded-full">
          {
            isApproveQuoteLoading ?
              <Spinner /> :
              'Approve Quote'
    ...
    {
      accountAllowance && accountAllowance > 0 && AccountActionType.DEPOSIT === accountActionType ?
        <div onClick={() => deposit?.()} className="cursor-pointer border-2 border-emerald-600 hover:border-emerald-600 p-2 py-3 col-span-3 font-semibold text-sm text-white text-center rounded-full">
          {
            isDepositLoading ?
              <Spinner /> :
              'Deposit'
    ...
    {
      AccountActionType.WITHDRAW === accountActionType ?
        <div onClick={() => withdraw?.()} className="cursor-pointer border-2 border-emerald-600 hover:border-emerald-600 p-2 py-3 col-span-3 font-semibold text-sm text-white text-center rounded-full">
          {
            isWithdrawLoading ?
              <Spinner /> :
              'Withdraw'
...
export const CreateAccount = () => {
  ...
  const { isLoading, newAccount } = useAccountFactory(accountFactoryContract[0], accountFactoryContract[1], chain?.id);

  return <>
    {
      isConnected ?
        <div onClick={() => newAccount?.()} className="cursor-pointer border-2 border-orange-500 hover:border-orange-600 p-2 py-3 col-span-3 font-normal text-md text-white text-center rounded-full">
          {
            isLoading ?
              <Spinner /> :
...
const useAccountFactory = (accountFactoryAddr: Address | undefined, abi: any, chainId: number | undefined) => {
