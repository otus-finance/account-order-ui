...
export const TradeMarket = () => {
  ...
  const handleExecuteMultiTrade = useCallback(async () => {
    ...
    const trades = await Promise.all(strikes.map(async (strike: LyraStrike) => {
  ...
  return <>
    ...
    <div className="col-span-1 px-4">
      ...
      <div className='py-6'>
        ...
        {
          isConnected && chain?.id === selectedChain?.chainId && quoteAsset && !quoteAsset.tradeAllowance.isZero() && !quoteAsset.balance.isZero() &&
          <div onClick={() => handleExecuteMultiTrade()} className="cursor-pointer border-2 border-emerald-600 hover:bg-emerald-600 bg-zinc-900 p-2 py-3 col-span-3 text-sm font-normal text-white text-center rounded-full">
        ...
        {/* wallet connected but wrong chain */}
        {
          ...
          <div onClick={openChainModal} className="cursor-pointer border-2 border-zinc-700 hover:border-emerald-600 p-2 py-3 col-span-3 font-normal text-sm text-white text-center rounded-full">
            Wrong network
          </div>
        }
