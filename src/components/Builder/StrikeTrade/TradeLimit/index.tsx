...
const TradeLimitActions = () => {
	...
	const { accountOrder, accountBalance, accountAllowance, order, setOrder, placeOrder } =
		useAccountOrderContext();
	...
	const setOrderWithValues = useCallback(() => {
		if (strikes.length === 1 && strikes[0] && order) {
			...
			if (
				!(order.size.eq(size) && order.strikeId.eq(_strikeId) && order.optionType == _optionType)
			) {
				setOrder({
					...order,
					market: _market,
