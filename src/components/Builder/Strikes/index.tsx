		...
		<div className="col-span-3 sm:col-span-3 px-6 pb-6 grid grid-cols-6">
			...
			<div className="sm:col-end-7 sm:col-span-2 col-start-1 col-end-7">
				<div className="col-span-1 grid grid-cols-3 gap-3 mt-2">
					{selectedStrategy && builderType === BuilderType.Builder && (
						<div
							onClick={() => handleBuildNewStrategy(!isBuildingNewStrategy)}
							className="mt-4 cursor-pointer border border-zinc-800 hover:border-emerald-600 bg-zinc-900 p-2 col-span-3 font-semibold text-xs text-white text-center rounded-2xl"
						>
							{isBuildingNewStrategy ? "Hide" : "View More Strikes"}
						</div>
					)}
				</div>
			</div>
			...
			{showStrikesSelect && builderType === BuilderType.Builder && (
				<motion.div
					...
					animate={showStrikesSelect ? "open" : "closed"}
					variants={{
						open: { opacity: 1, x: 0 },
