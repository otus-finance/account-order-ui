...
export const FooterNav = () => {
	return (
		<footer className="dark:bg-inherit mt-auto ">
			<div className="mx-auto py-12 px-4 sm:px-6 md:flex md:items-center md:justify-between lg:px-8">
				<div className="flex justify-center space-x-6 md:order-2 text-sm">
					{navigation.map((item) => (
						<a
							key={item.name}
							href={item.href}
							className="dark:text-zinc-200 hover:dark:text-zinc-500"
						>
							{item.name}
						</a>
					))}
				</div>
				<div className="flex justify-center space-x-6 mt-8 md:order-1 md:mt-0">
					{social.map((item) => (
						<a
							key={item.name}
							href={item.href}
